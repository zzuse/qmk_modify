...
void print_combo(combo_t *combo) {
    uprintf("combo keys %u, keycode %u, state %u\n", combo->keys[0], combo->keycode, combo->state);
}

static bool process_single_combo(combo_t *combo, uint16_t keycode, keyrecord_t *record, uint16_t combo_index) {
    uprintf("process_single_combo\n");
    uint8_t  key_count = 0;
    uint16_t key_index = -1;
    _find_key_index_and_count(combo->keys, keycode, &key_index, &key_count);
    ...
    if (record->event.pressed && key_is_part_of_combo) {
        uprintf("down Combo KL: kc: 0x%04X, col: %u, row: %u, pressed: %b, time: %u, interrupt: %b, tap_count: %u\n", keycode, record->event.key.col, record->event.key.row, record->event.pressed, record->event.time, record->tap.interrupted, record->tap.count);
...
#    endif
            ...
            uprintf("1 release Combo KL: kc: 0x%04X, key_index: %u, combo_index: %u, pressed: %b, time: %u, interrupt: %b, tap_count: %u\n", keycode, key_index, combo_index, record->event.pressed, record->event.time, record->tap.interrupted, record->tap.count);
            /* last key released */
            release_combo(combo_index, combo);
            ...
            uprintf("2 release Combo KL: kc: 0x%04X, key_index: %u, combo_index: %u, pressed: %b, time: %u, interrupt: %b, tap_count: %u\n", keycode, key_index, combo_index, record->event.pressed, record->event.time, record->tap.interrupted, record->tap.count);
            /* first or middle key released */
            key_is_part_of_combo = true;

#ifdef COMBO_PROCESS_KEY_RELEASE
            // if there are combos, release should return false
            if (process_combo_key_release(combo_index, combo, key_index, keycode)) {
                release_combo(combo_index, combo);
            }
#endif
            ...
            // Deactive
            uprintf("3 release Combo KL: kc: 0x%04X, key_index: %u, combo_index: %u, pressed: %b, time: %u, interrupt: %b, tap_count: %u\n", keycode, key_index, combo_index, record->event.pressed, record->event.time, record->tap.interrupted, record->tap.count);
